...
class TestAdminNamespaceWS(unittest.TestCase):
    ...
    def fetch_peer_count_and_peers(self):
        ...
        peer_count, error = Utils.call_ws(self.endpoint, method_net_peerCount, [], self.log_path)
        self.assertIsNone(error)

        method_admin_peers = f"{self.ns}_peers"
        peers, error = Utils.call_ws(self.endpoint, method_admin_peers, [], self.log_path)
        self.assertIsNone(error)
    ...
    def create_params_for_starting_rpc(self):
        ...
        if TestAdminNamespaceWS.runtime_apis_csv:
            apis = TestAdminNamespaceWS.runtime_apis_csv
        else:
            ...
            TestAdminNamespaceWS.runtime_apis_csv = apis
